...
function TwitAPI(_twit, _logger, _metrics, _user) {
  ...
  this.metricApiError = this.metrics.ApiError.withTags(metricsTags);
  this.metricApiFinished = this.metrics.ApiFinished.withTags(metricsTags);
...
TwitAPI.prototype.apiHarness = function(method, api, params, limiter ) {
  ...
  return new RSVP.Promise(function(resolve, reject) {
    ...
    limiter.removeTokens(1, function(err, remainingRequests) {
      ...
      {
        if ( !_.isEmpty(err)) {
          _this.metricApiError.withTags(metricsTags).increment();
          reject(err);
        } else {
          _this.metricApiFinished.withTags(metricsTags).increment();
          resolve(data);
        }
      });
    });
...
TwitAPI.prototype.followersIDs = function() {
  var _this = this;
  return function(user, cursor) {
    return _this.apiHarness('get', 'followers/ids', { user_id: user.id_str, cursor: cursor, count: 5000, stringify_ids: true }, _this.limiters.followersIDsLimiter);
  };
};
...
function setAppRateLimiters(){
  return {
    ...
    followersIDsLimiter: per15MinRateLimiter(15),
