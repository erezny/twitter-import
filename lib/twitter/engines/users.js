  ...
  engine.once('dbready', function() {
    ...
    usersWorker.queryFriends.cursor =
      singleCursor( {
        ...
        'locks.query_friends': 0,
        'locks.query_user': 0,
        friends_count: { $gt: 0, $lt: 5000 * 180 },
    ...
    usersWorker.queryFollowers.cursor =
      singleCursor( {
          ...
          'locks.query_followers': 0,
          'locks.query_user': 0,
          followers_count: { $gt: 1, $lt: 5000 * 180 }
        }, [
    ...
    usersWorker.expandFriends.cursor =
      singleCursor( {
          ...
          'locks.expand_friends': 0,
          'locks.query_user': 0,
          friends_count: { $gt: 0, $lt: 200 * 24 * 60 * 2 },
          followers_count: { $gt: 0 }
        }, [
          [ 'internal.expand_friends', -1 ],
          [ 'friends_count', 1 ],
    ...
    usersWorker.expandFollowers.cursor =
      singleCursor( {
        ...
        'locks.expand_followers': 0,
        'locks.query_user': 0,
        followers_count: { $gt: 1, $lt: 200 * 4 * 60 * 2 }
      }, [
        [ 'internal.expand_followers', -1 ],
        [ 'followers_count', 1 ],
      ]);
      ...
      function(err, result)
      {
        ...
        } else
        {
          ...
          twitter.controller.lockUser(result, function(err, result) {
            ...
            logger.info('queryFriends: %s', result.value.screen_name);
      ...
      function(err, result)
      {
        ...
        } else
        {
          ...
          twitter.controller.lockUser(result, function(err, result) {
            ...
            {
              logger.info('queryFollowers: %s', result.value.screen_name);
...
function countExistingUsers(user, users, profile_count, type){
  twitter.controller.countExistingUsers(users, function(count, query) {
    var usernameWidth = 20;
    var padding = " ".repeat(usernameWidth - user.name.length);
    logger.info("%s%s\t %d\t queried \t %d\t listed\t %d\t %s loaded",
            user.name, padding, count, users.length, profile_count, type);
  });
}

function countExistingFriends(user){
  countExistingUsers(user, user.friends, user.friends_count, "friends");
}

function countExistingFollowers(user){
  countExistingUsers(user, user.followers, user.followers_count, "followers");
}
