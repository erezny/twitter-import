    ...
    // TODO remove once database is persistent
        ...
        twitter.controller.collection.find(
          {
            ...
            'locks.query_user': 0,
            friends_count: {$gt : 10},
            friends_count: {$lt : 5000 * 60 } // 300000 in 60 minutes
          }
        )
        ...
        twitter.controller.collection.find(
          {
            ...
            followers_count: {$lt : 5000 * 60 } // 300000 in 60 minutes
          }
        )
        ...
        twitter.controller.collection.find(
          {
            ...
            'locks.query_user': 0,
            friends_count: {$gt : 10},
            friends_count: {$lt : 200 * 60 } // 12000 in 60 minutes
          }
        )
        .sort({friends_count: 1})
        .limit(1);
      ...
      usersWorker.expandFollowers.cursor =
        twitter.controller.collection.find(
        {
          ...
          'locks.query_user': 0,
          followers_count: {$gt : 10},
          followers_count: {$lt : 200 * 60 } // 12000 in 60 minutes
        }
      )
    ...
    // TODO Result && Parent, Result && ! parent
      if (result)
      {
        if (result.locks.query_user > 1){
          ...
          //TODO this functionality needs to be modeled and verified.
          setTimeout(function() {
