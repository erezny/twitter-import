    ...
    // TODO remove once
        ...
        twitter.controller.collection.find(
          {
            ...
            'locks.query_user': 0,
            friends_count: {$gt : 0}
          }).sort({'state.expand_friends': -1, 'internal.expand_friends': -1, friends_count: 1}
        ).limit(10);
      ...
      usersWorker.queryFollowers.cursor =
        twitter.controller.collection.find(
          ...
          }).sort({ 'state.expand_followers': -1, 'internal.expand_followers': -1, followers_count: 1}
        ).limit(10);
        ...
        twitter.controller.collection.find(
          {
            ...
            'locks.query_user': 0,
            friends_count: {$gt : 0}
          }).sort({'state.query_friends': 1, 'internal.expand_friends': -1, friends_count: 1}
        ).limit(4)
      ...
      usersWorker.expandFollowers.cursor =
        twitter.controller.collection.find(
        {
          ...
          'locks.query_user': 0,
          followers_count: {$gt : 0}
        }).sort({'state.query_followers': 1, 'internal.expand_followers': -1, followers_count: 1}
      ).limit(4);
  ...
  module.exports.get_next_expandFollowers = function()
  {
    ...
    usersWorker.expandFollowers.cursor.next(
      function(err, result)
      {
        ...
        if (err)
        {
          logger.debug('error querying next expandFollowers: %j', err);
        }
        else if (! result)
        {
