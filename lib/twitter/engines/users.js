  ...
  engine.once('dbready', function() {
    ...
    usersWorker.queryFollowers.cursor =
      singleCursor( {
          ...
          'locks.query_followers': 0,
          'locks.query_user': 0,
          'followers_count': { $gt: 0, $lt: 2000000 },
        }, [
    ...
    usersWorker.expandFollowers.cursor =
      singleCursor( {
        ...
        'locks.expand_followers': 0,
        'locks.query_user': 0,
        'followers_count': { $gt: 0, $lt: 1000000 },
      }, [
        [ 'internal.expand_followers', -1 ],
        [ 'followers_count', 1 ],
      ]);
...
//reduced from 2173 to 2038
function pruneUseless(user){
...
//  twitter.controller.countExistingUsers(users, function(count, query) {
    ...
    // logger.info("%s\t%s%s\t %d\t listed\t %d\t %s loaded",
    //         note, user.screen_name, padding, users.length, profile_count, type);
//  });
}
