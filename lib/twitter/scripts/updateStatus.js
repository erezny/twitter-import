...
function testQueryFriends(data){
  if ( Array.isArray(data.friends) &&
    ...
    logger.info('set query friends %d/%d \t%s', listed, user.friends_count, data.screen_name);
...
function testQueryFollowers(data){
  if ( Array.isArray(data.followers) &&
    ...
    logger.info('set query followers %d/%d \t%s', listed, user.followers_count, data.screen_name);

    logger.info("querying followers on %s", );
  ...
  return new RSVP.Promise( function (resolve, reject) {
    if ((data.friends.length || 0) > 0 && data.internal.expand_friends > 0 && data.friends.length < 10000){
      ...
      .then(function(results) {
        if ( results < data.friends.length * 0.9 ) {
          logger.info("expanding %d friends on %s",results, data.friend_count, data.screen_name);
  ...
  return new RSVP.Promise( function (resolve, reject) {
    if ((data.followers.length || 0) > 0 && data.internal.expand_followers > 0 && data.followers.length < 10000){
      ...
      countExistingUsers(data.followers).then(function(results) {
        if (results < data.followers.length * 0.9) {
          logger.info("expanding %d/%d followers on %s",results, data.followers.length , data.screen_name);
