...
function createRelationships(result) {
  return new RSVP.Promise( function (resolve, reject) {
    ...
    relationshipPromises.AllSettled(resolve);
  });

}

function nextTwitterUser(data){
  openQueries--;
...
var debugLimiter = new RateLimiter(1, (1 / 25) * 1000 );

function findUserByIDStr(id_str) {
  return new RSVP.Promise( function (resolve, reject) {
    logger.trace('finding node id_str:%s', id_str)
      ...
      debugLimiter.removeTokens(1, function(err, remainingRequests) {
        neo4j.find( { id_str: id_str }, false, "twitterUser" ,
          ...
          if (results.length > 0) {
            logger.debug('found node:%d id_str:%s', results[0].id, id_str)
            resolve(results[0]);
          } else {
