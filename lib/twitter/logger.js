...
module.exports = {
  ...
  // }
    ...
    setInterval(this.countRemaining, 5*60*1000);
  },
  ...
  countRemaining: function(){
    module.exports.collection.count({$or: [{'state.query_followers': true},{'state.query_friends': true},{'state.expand_followers': {$gt: 0}},{'state.expand_friends': {$gt: 0}}] }, function(err, results){
      messageBuffer.Total.push(results);
      if (messageBuffer.Total.length > 29){
