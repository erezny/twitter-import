...
TwitterNeo4j.prototype.queryRunner = function(query, _retry) {
  ...
  return new RSVP.Promise(function(resolve, reject) {
    var operation = this.neo4j.operation('transaction/commit', 'POST', query);
    this.neo4j.call(operation, function(err, neo4jresult, neo4jresponse) {
      if (!_.isEmpty(err)){
        this.logger.error("query error: (%d) %j\nquery: %j", retry, err, query);
        this.metrics.TxnError.increment();
        ...
        this.logger.trace(neo4jresult, neo4jresponse);
        this.metrics.TxnFinished.increment();
        resolve();
      }
    });
  });
...
TwitterNeo4j.prototype.saveFriendsIDs = function(user, friendsIDs) {
  this.logger.trace(user);
  var query = {
    statements: [
      {
