...
function queryRunner(query, retry){
  return new RSVP.Promise(function(resolve, reject) {
    ...
    neo4j.call(operation, function(err, neo4jresult, neo4jresponse) {
      if (!_.isEmpty(err)){
        logger.error("query error: (%d) %j", retry, err);
        metrics.TxnError.increment();
        if (err.code == 'ECONNRESET'){
          resolve(queryRunner(query, +retry + 1));
        } else {
          reject(err);
        }
      } else {
...
function resetListSubscriptions(user) {
  logger.trace(user);
  var query = {
    statements: [
      {
...
function saveListSubscriptions(user, lists) {
      var query = {
        statements: [
          {
            statement: "merge (u:twitterUser { id_str: {user}.id_str }) " +
...
module.exports = function(_neo4j, _logger, _metrics) {
  ...
  return {
    ...
    saveListSubscriptions: saveListSubscriptions,
    resetListSubscriptions: resetListSubscriptions
  };
};
