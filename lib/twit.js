...
module.exports = function(logger, metrics, userAuth) {
  var T;

  var auth_token = Object.assign({}, tokens);
  if (!_.isEmpty(userAuth)){
    auth_token.app_only_auth = false;
    auth_token.access_token = userAuth.acces_token;
    auth_token.access_token_secret = userAuth.access_token_secret;
  }

  T = new Twit(auth_token);

  T.queries = new TwitterQueries(T, logger, metrics, userAuth);
  return T;
};
