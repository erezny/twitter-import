...
function Twitter(_logger, _metrics, userAuth) {
  ...
  if (tokens.consumer_key.length > 0){
    this.logger.error("Twitter API Consumer key blank");
  }
  if (tokens.consumer_secret.length > 0){
    this.logger.error("Twitter API Consumer secret blank");
  }
