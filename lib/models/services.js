...
function importFollowersIDs(T, startNode){
  ...
  var processed = 0;
  var runFunction = function(node) {
    logger.debug("queryFollowersIDs %s, currently %d/%d", node.n.screen_name, node.followers_imported_count, node.n.followers_count);
    ...
    var saveFunction = function(user, results) {
      ...
      logger.trace(results);
      if (results.ids.length > 0){
        logger.debug("found %d followers of user %s", results.ids.length, user.screen_name );
        saved += results.ids.length;
      }
      if (saved === 0){
    ...
    return T.queries.pagedAPIQuery( node.n , apiFunction, saveFunction).then(function() {
      processed += 1;
      logger.debug("processed %d nodes", processed);
      logger.info("found %d followers of user %s", saved, node.n.screen_name );
    }).catch( function(err) {
      if (err.message == 'Not authorized.'){
        logger.info("found %d followers of user %s", saved, node.n.screen_name );
      } else {
        logger.error("unknown Error querying followers of %s: %j", node.n.screen_name, err);
      }
    });
  };
  var checkFunction = function(nodes) {
