...
function importUserListOwnership(){
  ...
  var operation = neo4j.operation('node/7307455/paged/traverse/node?pageSize=1&leaseTime=6000', 'POST', userGenericTemplate("listOwnership",since_timestamp, 3) );
...
function importListMembers(){
  ...
  var runFunction = function(nodes) {
    var list = nodes.n;
    logger.info("queryListMembers %s", list.full_name);
    var saved = 0;
...
function importUserListSubscriptions(){
  var checkNodeQuery = checkFriendsCount();
  var since_timestamp = 1461116903347;
  var operation = neo4j.operation('node/7307455/paged/traverse/node?pageSize=1&leaseTime=6000', 'POST', userGenericTemplate("listSubscriptions",since_timestamp, 3) );
  ...
  var runFunction = function(nodes) {
    ...
    return neo4j.twitter.resetListSubscriptions(user).then(function() {
      return apiFunction( user ).then(function(results) {
        logger.trace(results);
        if (results.lists.length > 0){
          logger.info("found %d lists subscribed to by %s", results.lists.length, user.screen_name )
        }
        return saveFunction(user, results.lists).then(function() {
          processed += 1;
          logger.debug("processed %d nodes", processed);
