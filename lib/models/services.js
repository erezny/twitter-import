...
function userListOwnershipTemplate(property_type, since_timestamp, depth){
  return {
    "order": "breadth_first",
    "return_filter": {
      "body":
...
function importFriendsIDs(){
  ...
  var since_timestamp = (new Date).getTime();
  var operation = neo4j.operation('node/7307455/paged/traverse/node?pageSize=1&leaseTime=6000', 'POST', queryTemplate("friends",since_timestamp, 25000, 3) );
...
function importUserListOwnership(){
  var checkNodeQuery = checkFriendsCount();
  var since_timestamp = 1461116903347;
  var operation = neo4j.operation('node/7307455/paged/traverse/node?pageSize=1&leaseTime=6000', 'POST', userListOwnershipTemplate("listOwnership",since_timestamp, 3) );
...
function importListMembers(){
  ...
  var since_timestamp = 1461116903347;
  var operation = neo4j.operation('node/7307455/paged/traverse/node?pageSize=1&leaseTime=6000', 'POST', listMembersTemplate("members",since_timestamp, 3) );
  var apiFunction = T.queries.listMembers;
  var saveFunction = neo4j.twitter.saveListMembers;
  var processed = 0;
  var runFunction = function(nodes) {
    var list = nodes.n;
    logger.debug("queryListMembers %s", list.full_name);
    return neo4j.twitter.resetListMembers(list).then(function() {
      return apiFunction( list ).then(function(results) {
        logger.trace(results);
        if (results.users.length > 0){
          logger.info("found %d users in list %s", results.users.length, list.full_name );
        }
        return saveFunction(list, results.users).then(function() {
          processed += 1;
          logger.debug("processed %d nodes", processed);
        });
      });
