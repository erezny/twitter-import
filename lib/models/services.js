...
function checkFollowersCount(){
  return `match (n:twitterUser) where id(n) in {nodes}
    optional match followers=(n)<-[:follows]-(l:twitterUser)
    with n, size(collect (distinct l)) as followers_imported_count
    where followers_imported_count <> n.followers_count
    return n, followers_imported_count` ;
}
...
function runGraphTraversal(startNode, pageSize, traversal_query, page_callback, description){
    if (typeof(startNode) == 'number') { startNode = util.format('%d', startNode); }
    ...
    return traverse (page_callback, operation, description);
}

var traverseLimiter = new RateLimiter(1, 100);
function traverse(runPerPage, _operation, description){
  return new RSVP.Promise(function(resolve, reject) {
    var keep_running = new _.CatchSigterm();
    runNextPage(_operation, _operation);

    function runNextPage(operation, original_operation){
      logger.trace("run");
        ...
        neo4j.call(operation, function(err, results, response) {
          if (!_.isEmpty(err)){
            if (err.neo4jError && err.neo4jError.fullname == 'org.neo4j.graphdb.NotFoundException') {
              logger.info("restarting traversal %s", description);
              setTimeout(runNextPage, 1 * 60 * 1000, original_operation, original_operation);
            } else {
              reject(err);
            }
          } else {
            ...
            runPerPage(results).then(function() {
              if (keep_running.get()){
                process.nextTick(runNextPage, operation, original_operation);
              } else {
...
function importFriendsIDs(T, startNode){
  ...
  var since_timestamp = (new Date).getTime() - 14 * 24 * 60 * 60 * 1000 ;
  var traversal_query = queryTemplate("friends",since_timestamp, 25000, 4) ;
  var apiFunction = T.queries.friendsIDs();
  var processed = 0;
  var runFunction = function(node) {
    logger.debug("queryFriendsIDs %s, currently %d/%d", node.n.screen_name || node.n.id_str, node.friends_imported_count, node.n.friends_count || 0);
    var saved = 0;
    var saveFunction = function(user, results) {
      logger.trace(results);
      var resetQuery = null;
      if (results.ids.length > 0){
        logger.debug("found %d friends of user %s", results.ids.length, user.screen_name || user.id_str );
        saved += results.ids.length;
      }
      if (saved === 0){
        resetQuery = neo4j.twitter.resetFriends(node.n);
      }
      return neo4j.twitter.saveFriendsIDs(user, results.ids, resetQuery);
    };
    return T.queries.pagedAPIQuery( node.n , apiFunction, saveFunction).then(function() {
      processed += 1;
      logger.debug("processed %d nodes", processed);
      logger.info("found %d friends of user %s", saved, node.n.screen_name || node.n.id_str );
    }).catch( function(err) {
      if (err.message == 'Not authorized.'){
        logger.info("found %d friends of user %s", saved, node.n.screen_name || node.n.id_str );
      } else {
        logger.error("unknown Error querying friends of %s: %j", node.n.screen_name || node.n.id_str, err);
      }
    });
  };
  ...
  return runGraphTraversal(startNode, 1, traversal_query, checkFunction, 'importFriendsIDs');
}

function importFollowersIDs(T, startNode){
  logger.trace(startNode);
  var checkNodeQuery = checkFollowersCount();
  var since_timestamp = (new Date).getTime() - 14 * 24 * 60 * 60 * 1000 ;
  var traversal_query = queryTemplate("followers",since_timestamp, 25000, 4) ;
  var apiFunction = T.queries.followersIDs();
  var processed = 0;
  var runFunction = function(node) {
    logger.debug("queryFollowersIDs %s, currently %d/%d", node.n.screen_name || node.n.id_str, node.followers_imported_count, node.n.followers_count || 0);
    ...
    var saveFunction = function(user, results) {
      ...
      logger.trace(results);
      if (results.ids.length > 0){
        logger.debug("found %d followers of user %s", results.ids.length, user.screen_name || user.id_str );
        saved += results.ids.length;
      }
      if (saved === 0){
        resetQuery = neo4j.twitter.resetFollowers(node.n);
      }
      return neo4j.twitter.saveFollowersIDs(user, results.ids, resetQuery);
    };
    return T.queries.pagedAPIQuery( node.n , apiFunction, saveFunction).then(function() {
      processed += 1;
      logger.debug("processed %d nodes", processed);
      logger.info("found %d followers of user %s", saved, node.n.screen_name || node.n.id_str );
    }).catch( function(err) {
      if (err.message == 'Not authorized.'){
        logger.info("found %d followers of user %s", saved, node.n.screen_name || node.n.id_str );
      } else {
        logger.error("unknown Error querying followers of %s: %j", node.n.screen_name || node.n.id_str, err);
      }
    });
  };
  var checkFunction = function(nodes) {
  ...
  return runGraphTraversal(startNode, 1, traversal_query, checkFunction, 'importFollowersIDs');
}

function importUsers(T, startNode){
  logger.trace(startNode);
  var since_timestamp = (new Date).getTime() - 28 * 24 * 60 * 60 * 1000 ;
  ...
  var runFunction = function(nodes) {
    ...
    return apiFunction(id_str_list).then(function(users) {
      neo4j.twitter.saveUsers(users);
    }).then(function() {
  ...
  return runGraphTraversal(startNode, 100, traversal_query, runFunction, 'importUsers');
}
...
function importUserListOwnership(T, startNode){
  ...
  var checkNodeQuery = checkFriendsCount();
  var since_timestamp = (new Date).getTime() - 28 * 24 * 60 * 60 * 1000 ;
  var traversal_query = userGenericTemplate("listOwnership",since_timestamp, 4);
  var apiFunction = T.queries.userListOwnership();
  var processed = 0;
  var runFunction = function(nodes) {
    var user = nodes[0].data;
    logger.debug("queryUserListOwnership %s", user.screen_name || user.id_str);
    return neo4j.twitter.resetListOwnerships(user).then(function() {
      return apiFunction( user ).then(function(results) {
        ...
        if (results.lists.length > 0){
          logger.info("found %d lists owned by %s", results.lists.length, user.screen_name || user.id_str );
        }
        return neo4j.twitter.saveLists(user, results.lists).then(function() {
          processed += 1;
          logger.debug("processed %d nodes", processed);
        });
      });
  ...
  return runGraphTraversal(startNode, 1, traversal_query, runFunction, 'importUserListOwnership');
}
...
function importListMembers(T, startNode){
  ...
  var since_timestamp = (new Date).getTime() - 28 * 24 * 60 * 60 * 1000 ;
  var traversal_query = listMembersTemplate("members", since_timestamp, 4);
  var processed = 0;
  ...
  return runGraphTraversal(startNode, 1, traversal_query, checkFunction, 'importListMembers');
}
...
function importUserListSubscriptions(T, startNode){
  ...
  var runFunction = function(nodes) {
    var saved = 0;
    var user = nodes[0].data;
    logger.debug("ListSubscriptions %s", user.screen_name || user.id_str);
    return neo4j.twitter.resetListSubscriptions(user).then(function() {
      return apiFunction( user ).then(function(results) {
        logger.trace(results);
        if (results.lists.length > 0){
          logger.debug("found %d lists subscribed to by %s", results.lists.length, user.screen_name || user.id_str );
          saved += results.lists.length;
        }
        return neo4j.twitter.saveListSubscriptions(user, results.lists).then(function() {
          processed += 1;
          logger.debug("processed %d nodes", processed);
          if (saved > 0){
            logger.info("found %d lists subscribed to by %s", saved, user.screen_name || user.id_str);
          }
        });
      });
    }).catch(function(err) {
  ...
  return runGraphTraversal(startNode, 1, traversal_query, runFunction, 'importUserListSubscriptions');
}

function runAppImports() {
  var T = new Twit(logger, metrics);
  return neo4j.twitter.getVIPNode().then(function(startNode) {
    return [
      importFriendsIDs(T, startNode),
      importFollowersIDs(T, startNode),
...
function runAllUserImports() {
  return new RSVP.Promise(function(resolve, reject) {
    findOuathInfos().then(function (oauths) {
      logger.trace(oauths);
      var jobs = runUserImports(oauths[0]);
      resolve(jobs);
    });
  });
}
