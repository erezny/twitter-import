...
const influxConnectString = util.format("%s://%s:%s@%s:%d/write?db=%s",
  influxServer.protocol, influxServer.username, influxServer.password,
  influxServer.host, parseInt(influxServer.port), influxServer.database);

module.exports = {
      init: function(prefix, tags, logger) {
